...
# 2) Wait until the DB is truly ready (pg_isready + SELECT 1)
...
# --- helper to run SQL from host via stdin --------------------------------
run_sql () {
  local file="$1"
  if [[ -f "$file" ]]; then
    echo "2) Running $file"
...
# --- refresh MView (if exists) -------------------------------------------
echo "3) Refresh materialized view (if present)…"
docker exec -i "$CONTAINER" psql -U "$DB_USER" -d "$DB_NAME" -v ON_ERROR_STOP=1 -q -c \
"DO \$\$BEGIN
...
docker exec -i "$CONTAINER" psql -U "$DB_USER" -d "$DB_NAME" -Atq -c \
  "COPY (SELECT * FROM staff_locations_view) TO STDOUT WITH CSV HEADER" \
  > data/staff_locations_view.csv 2>/dev/null || echo "   staff_locations_view not found — skip"
